...
class PhpSerial
{
    ...
    public function deviceSet($device)
    {
        if ($this->_dState !== SERIAL_DEVICE_OPENED) {
            if ($this->_os === "linux") {
                ...
                if
				   (preg_match("@^COM(\\d+):?$@i", $device, $matches)and $this->_exec(exec("mode " . $device . " xon=on BAUD=9600")) === 0)
                {
    ...
    public function deviceOpen($mode = "r+b")
    {
        ...
        //if ($this->_os === "windows")$this->_dHandle = @fopen($this->_winDevice, $mode);
        //else
        $this->_dHandle = @fopen($this->_device, $mode);

        print $this->_dHandle.", ".$this->_device;
